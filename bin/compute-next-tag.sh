...
#
# Tags look like `v<Plausible Analytics version>-<release>`:
#
# - if defaults/main.yml points at a Plausible Analytics version that has never
#   been released, the release counter restarts at 0 (`v3.2.2-0`)
# - otherwise the counter is incremented (`v3.2.2-1`), but only if something
...
#
# `git diff` tolerates paths that do not exist, so `handlers` and `vars` are
# listed even though this role has neither yet.
role_defining_paths=(
	'defaults'
	'handlers'
	'meta'
...
version="$(sed -nE 's|^plausible_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Plausible Analytics version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v3.2.1`),
